...
\jb{Following paragraph removed.}
\end{enumerate}
...
whereas the latter program should either not be executable or lead to a state where $p$ is undeterministically true or false.
\ah{
...
\textbf{JB} Est-ce mieux maintenant ?
}
...
\end{align*}
\caption{Useful programs, for all finite $P \subseteq \propset$ and all programs $\pi_1$ and $\pi_2$.
...
The more liberal semantics of \cite{HerzigEtal-Ijcai19} makes that the absence of cross-interactions of parallel actions %in a parallel plan
has to be ensured `by hand' when expressing parallel plans in the logic,
namely by means of explicit tests\jb{
Des tests dans le langage de la logique ou des tests hors de la logique ?
}.
...
is not executable in our semantics: each subprogram requires writability of $\mathsf{OnTable}(b)$ to be executable. 
