...
%\usepackage{pdfpages} POUR : %\includepdf[page={1,2,3,4,5,6}]{reviews123withresponse.pdf}
...
Distinguishing the variables that can be tested %is a natural way to separate the domain of actions of each subprogram of a parallel composition.
...
The present paper is a more elaborate version of %a paper that was presented at DaL\'i 2019
...
If there is $P \subseteq \propset$ such that
$\modl_1 \modinter P = \modl_1' \modinter P$,
$\modl_2 \modinter P = \modl_2' \modinter P$,
$\propsetOf \phi \subseteq P$ and $\propsetOf \pi \subseteq P$ then the following hold:
\begin{align*}
\modl_1 \models \phi &\text{ iff } \modl_1' \models \phi \\
\modl_1 \intPgm \pi \modl_2 &\text{ iff } \modl_1' \intPgm \pi \modl_2'
\end{align*}
\end{lemma}
\begin{proof}[sketch]
The proof is by a straightforward simultaneous
...
\end{align*}
\caption{Useful programs and formulas, for all finite $P \subseteq \propset$ and all programs $\pi_1$ and $\pi_2$.
...
We have adopted a stricter stance on race conditions than in~\cite{HerzigEtal-Ijcai19} %\jb{Removed `the approach of' to fix overfull hbox.}
...
%\jb{Supp. de la phrase sur l'interleaving~: tout interleaving com\-men\-çant par les deux tests est exécutable.}
%In other words, $\mathsf{liftLeft}(b)$ and $\mathsf{liftRight}(b) $ cannot be interleaved. 
%, and can therefore not be executed in parallel.  
...
\begin{proof}
  ...
  \begin{figure}%[h]
    ...
    \begin{tabular}{lcl}
      ...
      %\input{figure-associativity1.tex} AAAAAAAAAAAAAAAAAAAAAAA
...
\end{tikzpicture}
      & ~ &
      %\input{figure-associativity2.tex} AAAAAAAAAAAAAAAAAAAAAAA
\begin{tikzpicture}[>=latex', join=bevel, initial text = , every node/.style=, scale=1.2]
  % States
  \node (m) at (0bp, 0bp) {$\modl$};
