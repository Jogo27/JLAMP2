...
As for writability, separating the variables that can be tested is a natural way to separate the domain of actions of each branch of a parallel composition.
Moreover, it permits additional usefull checks.
...
In the present work, we enforce that a variable can not be readable in one branch of a parallel composition if it is writable in the other branch.
...
\ref{item:subAllProgram} is proved from \ref{item:addAllProgram} by instanciating $\readset$,
...
For the reduction axiom for endogeneous test, it suffices to observe that
...
The cases for endogeneous tests and parallel compositions are similar to the ones in the proof of Theorem~\ref{theo:axiomatisationsound}.
