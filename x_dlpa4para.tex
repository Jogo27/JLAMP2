...
\newcommand{\progClean}{\mathsf{clean}}
...
For any model $\modl = \tuple{\readset, \writeset, \valuset}$ and $P \subseteq \propset$, we write
...
%The next lemma is standard and
will be used several times in our soundness proof. 
...
\Big)
\\
\progClean(P) =&\ \seqseq{p \in P} \seqseq{k \in \{1, 2\}} \Big(
...
\\&\ %
  ...
  \progClean\left(\propsetOf{\pi_1 \pll \pi_2}\right)
\end{align*}
...
$\readset \cup \valuset \subseteq P$,
$\splt \modl {\modl_1} {\modl_2}$ if and only if
...
The $\progClean(P)$ program discards read and write access to copies and auxiliary variables.
It is needed for the following lemma, which can be seen as an adaptation of lemma~\ref{theo:irrelevantVariables}
to the $\progFlatten$ program.
...
(2) set to false and made unreadable by $\progClean$.
\end{proof}

Finally, the $\progFlatten(\pi_1, \pi_2)$ program emulates the execution of the program $\pi_1 \pll \pi_2$ by
a sequential composition of the previous program.
...
The following lemma states that the emulation is faithful. This the main lemma of our soundness proofs.

\begin{lemma}\label{lem:pllequivalence}
Let $\modl$ and $\modl^{\sharp}$ be two models, $\pi_1$ and $\pi_2$ two programs, and $P = \propsetOf{\pi_1 \pll \pi_2}$.
The following statements are equivalent:
\begin{enumerate}
  \item \label{item:pllequivalence:par} There is a model $\modl'$ such that
        $\modl \intPgm{\pi_1 \pll \pi_2} \modl'$
        and $\modl' \modinter P = \modl^\sharp$.
  \item \label{item:pllequivalence:seq} There is a model $\modl''$ such that
        $\modl \intPgm{\progFlatten(\pi_1, \pi_2)} \modl''$
        and $\modl'' \modinter P = \modl^\sharp$.
\end{enumerate}
\end{lemma}
...
\ldia{ \progFlatten(\pi_1, \pi_2) } ~ \phi . $$
...
{\footnotesize
 \begin{enumerate}
 \item
$\tuple{\readset,\writeset,\valuset} \intPgm{ \progsplit(P) } 
 \tuple{
\readset  {\cup} \cp 1 {(\readset_1{\cap} P)} {\cup} \cp 2 {(\readset_2{\cap} P)} , 
\writeset {\cup} \cp 1 {(\writeset_1{\cap} P)} {\cup} \cp 2 {(\writeset_2{\cap} P)} , 
\valuset  {\cup} \cp 1 {(\valuset{\cap} P)} {\cup} \cp 2 {(\valuset{\cap} P)}
