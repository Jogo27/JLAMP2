...
The following lemma states formaly the important property of $\progmerge(P)$.
...
where $\valuset^\sharp$ is any subset of $P$ such that
...
The following lemma states that the emulation is faithful.
...
\end{lemma}
\begin{proof}[sketch]
\newcommand{\stored}{_\text{St}}
Let $P = \propsetOf{\pi_1 \pll \pi_2}$. By Lemmas~\ref{theo:irrelevantVariables} and~\ref{lem:progClean},
we can assume without loss of generality that $\modl \modinter P = \modl$ and $\modl' \modinter P = \modl'$.
...
\begin{enumerate}
  ...
  \item\label{pllequivalence:ltr:store}
        ...
        $\readset^+_2 = \readset^+_1 \cup \writeset\stored$,
        $\writeset^+_2 = \writeset^+_1 \cup \writeset\stored$,
        $\valuset^+_2 = \valuset^+_1 \cup \valuset\stored$,
        $\writeset\stored =
          \set{ \cpr k p \suchthat k \in \set{1,2}, p \in P} \cup
          \set{ \cpw k p \suchthat k \in \set{1,2}, p \in P}$, and
        $\valuset\stored =
